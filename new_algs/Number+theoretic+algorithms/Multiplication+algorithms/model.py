...
class NPI():
    def __init__(self, core, config, log_path, npi_core_dim=256, npi_core_layers=2, verbose=0, device='/cpu:0'):
        """
    ...
    def npi_core(self):
        ...
        # Reshape state_in
        s_in = tf.reshape(s_in, [-1, 1, self.state_dim])    # Shape: [bsz, 1, state_dim]
        ...
        for i in range(self.npi_core_layers):
            out = tf.keras.layers.LSTM(units=self.npi_core_dim, return_sequences=True,
                                                 stateful=True, return_state=True)(c)
            c = out[0]
            self.h_states[i] = tf.concat(out[1:], axis=-1)
            

        # Return Top-Most LSTM H-State
        top_state = tf.split(self.h_states[-1], 2, axis=1)[1]
    ...
    def terminate_net(self):
        ...
        """
        p_terminate = tf.keras.layers.Dense(units=2, kernel_regularizer=tf.keras.regularizers.l2)(self.h)
    ...
    def key_net(self):
        ...
        hidden = tf.keras.layers.Dense(self.key_dim, activation=tf.keras.activations.elu,
                                                kernel_regularizer=tf.keras.regularizers.l2)(self.h)
        key = tf.keras.layers.Dense(self.key_dim)(hidden)    # Shape: [bsz, key_dim]
    ...
    def argument_net(self):
        ...
        for i in range(self.num_args):
            arg = tf.keras.layers.Dense(units=self.arg_depth, kernel_regularizer=tf.keras.regularizers.l2,
