...
"""
    recursive implementation with memoization (2darray of alignments), with endgap penalty implementation, visualization available  
"""
...
def align_sequences(s1, s2, eg1, eg2, memCon, i, j):
    ...
    opt1[0] += ENDGAP if eg1 else (EXTENDEDGAP if opt1[3].startswith("-") else GAP)
    opt2[0] += ENDGAP if eg2 else (EXTENDEDGAP if opt1[1].startswith("-") else GAP)
