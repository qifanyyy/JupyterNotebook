# -*- coding: utf-8 -*-
"""
Created on Thu Jun 20 20:27:31 2019
@author: wmy
"""
...
# std colors
default_std_colors = {'DG0546':[27.818, -8.375, 8.282],
...
class MainFace(object):
    ...
    def __init__(self, root):
        ...
        button1 = tk.Button(self.face, text='背景主色选取', bg='#D2F99A', \
                            width=20, height=8, \
        ...
        label = tk.Label(self.face, text='多背景共有色选取软件', \
                      font=('System', 20), width=32, height=2, justify=tk.LEFT)
...
class SingleFace(object):
    ...
    def __init__(self, root):
        ...
        b2.place(x=7, y=158) 
        # std color select ui
        lstd = tk.Label(self.face, text='军标色文件：', \
                      font=('Arial', 12), width=16, height=2, justify=tk.LEFT)
        lstd.place(x=300, y=100)
        self.std_color_path = tk.StringVar()
        self.estd = tk.Entry(self.face, textvariable=self.std_color_path, width=35)
        self.estd.place(x=450+5, y=115)
        bstd = tk.Button(self.face, text='选择文件', width=8, height=1, command=self.select_std_color_path)
        bstd.place(x=720+5, y=105) 
        # default std colors
        self.std_colors = default_std_colors
        self.std_color_data_init()
        pass
    
    def std_color_data_init(self):
        self.colors = []
        self.labels = []
        for key, value in self.std_colors.items():
    ...
    def colour_classify(self, incolour, traindata, traincolour, k):
        '''训练的颜色个数'''
    ...
    def select_std_color(self, incolour):
        return self.colour_classify(incolour, self.colors, self.labels, 1)
    ...
    def select_std_color_path(self):
        path = askopenfilename()
        self.std_color_path.set(path)
        pass
    ...
    def main(self):
        # load std color
        std_color_path = self.estd.get()
        try:
            with open(std_color_path, 'r') as f:
                load_dict = json.load(f)
                pass
            pass
        except:
            self.state.set('Warning: 军标色文件载入失败')
            self.lstate.config(bg='#FFFF7F')
            self.face.update_idletasks()
            pass
        else:
            self.std_colors = load_dict
            self.std_color_data_init()
            pass
        pass
        # check input
        ...
        sorted_themes_areas = sorted(themes_areas, key=lambda x:(x[1]))
        # descending sort
        sorted_themes_descending = []
        for theme_area in sorted_themes_areas:
            theme = theme_area[0]
            sorted_themes_descending.append(theme)
            pass
        sorted_themes_descending = np.array(sorted_themes_descending)
        # bilateral sort
        sorted_themes_bilateral = []
        for i, theme in enumerate(sorted_themes_descending):
            if (i % 2) == 0:
                sorted_themes_bilateral.append(theme)
                pass
            else:
                sorted_themes_bilateral.insert(0, theme)
                pass
            pass
        sorted_themes_bilateral = np.array(sorted_themes_bilateral)
        # choose sort mode
        sorted_themes = sorted_themes_bilateral
        # show themes image
        uint8_themes = []
        ...
        image_LAB = cv2.cvtColor(image_RGB, cv2.COLOR_RGB2LAB)
        reduction_image = []
        for h in range(image_LAB.shape[0]):
            reduction_image.append([])
            for w in range(image_LAB.shape[1]):
                pix_data = image_LAB[h][w]
                pix_lab = [pix_data[0]*100/255, pix_data[1]-128, pix_data[2]-128]
                label = self.colour_classify(pix_lab, image_lab_data, labels, 1)
                pix_theme = themes[label]
                pix_theme_uint8 = np.uint8([pix_theme[0]*255/100, pix_theme[1]+128, pix_theme[2]+128])
                reduction_image[h].append(pix_theme_uint8)
                pass
            pass
        reduction_image = np.uint8(reduction_image)
        pic_array = cv2.cvtColor(reduction_image, cv2.COLOR_LAB2RGB)
        ...
        for theme, area in themes_areas:
            std = self.select_std_color(theme)
...
class DoubleFace(object):
    ...
    def __init__(self, root):
        ...
        self.e4.place(x=250+150+5, y=115)
        self.state = tk.StringVar() 
        self.state.set('正常')
        ...
        lstd = tk.Label(self.face, text='军标色文件：', \
                      font=('Arial', 12), width=16, height=2, justify=tk.LEFT)
        lstd.place(x=470, y=100)
        self.std_color_path = tk.StringVar()
        self.estd = tk.Entry(self.face, textvariable=self.std_color_path, width=10)
        self.estd.place(x=620+5, y=115)
        bstd = tk.Button(self.face, text='选择文件', width=8, height=1, command=self.select_std_color_path)
        bstd.place(x=720+5, y=105) 
        # default std colors
        self.std_colors = default_std_colors
    ...
    def main(self):   
        ...
        # sort themes
        themes_areas = list(zip(themes_1, clusters_area_1))
        sorted_themes_areas = sorted(themes_areas, key=lambda x:(x[1]))
        # descending sort
        sorted_themes_descending = []
        for theme_area in sorted_themes_areas:
        ...
        sorted_themes_bilateral = np.array(sorted_themes_bilateral)
        # choose sort mode
        sorted_themes_1 = sorted_themes_bilateral
        # show image
        uint8_themes_1 = []
        for theme in sorted_themes_1:
        ...
        self.state.set('色块图重构中。。。')
        self.face.update_idletasks()
        image_RGB = Image.open(image_1_path)
        w_resize = 128
        h_resize = 64
        image_RGB = image_RGB.resize((w_resize, h_resize))
        image_RGB = np.array(image_RGB)
        ...
        for h in range(image_LAB.shape[0]):
            ...
            for w in range(image_LAB.shape[1]):
                pix_data = image_LAB[h][w]
                pix_lab = [pix_data[0]*100/255, pix_data[1]-128, pix_data[2]-128]
                label = self.colour_classify(pix_lab, image_1_lab_data, labels, 1)
                pix_theme = themes_1[label]
                pix_theme_uint8 = np.uint8([pix_theme[0]*255/100, pix_theme[1]+128, pix_theme[2]+128])
                reduction_image[h].append(pix_theme_uint8)
                pass
            pass
        ...
        # sort themes
        themes_areas = list(zip(themes_2, clusters_area_2))
        sorted_themes_areas = sorted(themes_areas, key=lambda x:(x[1]))
        # descending sort
        sorted_themes_descending = []
        for theme_area in sorted_themes_areas:
        ...
        sorted_themes_bilateral = np.array(sorted_themes_bilateral)
        # choose sort mode
        sorted_themes_2 = sorted_themes_bilateral
        # show image
        uint8_themes_2 = []
        for theme in sorted_themes_2:
        ...
        self.state.set('色块图重构中。。。')
        self.face.update_idletasks()
        image_RGB = Image.open(image_2_path)
        w_resize = 128
        h_resize = 64
        image_RGB = image_RGB.resize((w_resize, h_resize))
        image_RGB = np.array(image_RGB)
        ...
        for h in range(image_LAB.shape[0]):
            ...
            for w in range(image_LAB.shape[1]):
                pix_data = image_LAB[h][w]
                pix_lab = [pix_data[0]*100/255, pix_data[1]-128, pix_data[2]-128]
                label = self.colour_classify(pix_lab, image_2_lab_data, labels, 1)
                pix_theme = themes_2[label]
                pix_theme_uint8 = np.uint8([pix_theme[0]*255/100, pix_theme[1]+128, pix_theme[2]+128])
                reduction_image[h].append(pix_theme_uint8)
                pass
            pass
        ...
        for i in range(n_clusters_1):
            ...
            for j in range(n_clusters_2):
                ...
                if deltaE <= color_tolerance:
                    ...
                    LAB2 = [L2, A2, B2]
                    LAB3 = [L3, A3, B3]
                    selected_std_color = self.select_std_color(LAB3)
                    # info
                    info = (LAB3, LAB1, Image_1_Area[i], LAB2, \
                            Image_2_Area[j], selected_std_color)
        ...
        for i in range(len(common_color_infos)):
            # std color index: 5
            selected_std_colors.append(common_color_infos[i][5])
            pass
        ...
        for num in std_colors_nums:
            ...
            for color in dict_selected_std_colors[str(num)]:
                ...
                for color_info in common_color_infos:
                    # std color index: 5
                    if color_info[5] == color:
                        index += 1
                        c3, c1, a1, c2, a2, sc = color_info
                        if c1 not in c1_list:
                            sca1 += a1
                            c1_list.append(c1)
                            pass
                        if c2 not in c2_list:
                            sca2 += a2
                            c2_list.append(c2)
                            pass
                        results.append((index, c3, c1, a1, c2, a2, sc))
                        pass
                    pass
                ...
                for result in results:
                    count += 1
                    i, c3, c1, a1, c2, a2, sc = result
                    a1 = round(100*a1, 3)
                    a2 = round(100*a2, 3)
                    info = ' '*2 + str(i) + ' '*(7-len(str(i)))
                    ...
                    if count<=1:
                        ...
                        self.results.append((i, c3, c1, a1, c2, a2, sc, sca1, sca2))
                        pass
                    else:
                        self.results.append((i, c3, c1, a1, c2, a2, sc, None, None))
                        pass
                    self.listbox.insert(tk.END, info)
                    self.face.update_idletasks()
                    pass
