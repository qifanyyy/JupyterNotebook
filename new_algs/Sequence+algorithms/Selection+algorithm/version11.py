...
scrollbar.place(x=765, y=368, height=368)

listbox = tk.Listbox(window, yscrollcommand=scrollbar.set, width=102, height=20)
...
def main():
    ...
    title = ' '*22 + 'LAB' + ' '*(48-3) + 'A' + ' '*48 + 'B' + ' '*26 + 'Std Color'
    ...
    for i in range(len(common_color)):
        ...
        info += ' '*(114-len(info))
        info += str(select_std_color(common_color[i]))
