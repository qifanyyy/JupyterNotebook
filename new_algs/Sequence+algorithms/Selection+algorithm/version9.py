...
def main():
    ...
    km1 = KMeans(n_clusters=n_clusters_1, max_iter=600)
    km1.fit(image_1_data)
    theme_1 = km1.cluster_centers_
    # show image
    ...
    km2 = KMeans(n_clusters=n_clusters_2, max_iter=600)
    km2.fit(image_2_data)
    theme_2 = km2.cluster_centers_
    # show image
    ...
    title = ' '*15 + 'LAB' + ' '*(35-3) + 'A' + ' '*42 + 'B' + ' '*39 + 'Std Color'
    ...
    for i in range(len(common_color)):
        ...
        ' '*(40-len(str(common_color[i]))) + \
        str(round(100*common_area[i][0], 2)) + '%' + \
        ' '*(40-1-len(str(round(100*common_area[i][0], 2)))) + \
        str(round(100*common_area[i][1], 2)) + '%' + \
        ' '*(40-1-len(str(round(100*common_area[i][1], 2)))) + \
        str(select_std_color(common_color[i]))
