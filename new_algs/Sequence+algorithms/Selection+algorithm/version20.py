...
def main():
    ...
    for theme in theme_1:
        uint8_theme_1.append([theme[0]*255/100, theme[1]+128, theme[2]+128])
        pass
    uint8_theme_1 = np.array(uint8_theme_1)
        
    pic_array = cv2.cvtColor(np.uint8(uint8_theme_1.reshape(1, len(uint8_theme_1), 3)), cv2.COLOR_LAB2RGB)
    ...
    for theme in theme_2:
        uint8_theme_2.append([theme[0]*255/100, theme[1]+128, theme[2]+128])
        pass
    uint8_theme_2 = np.array(uint8_theme_2)
    
    pic_array = cv2.cvtColor(np.uint8(uint8_theme_2.reshape(1, len(uint8_theme_2), 3)), cv2.COLOR_LAB2RGB)
    pic_array = make_image(pic_array[0])
    pic = Image.fromarray(pic_array.astype('uint8')).convert('RGB')
    img = ImageTk.PhotoImage(pic)
        ...
        for color in dict_colors[key]:
            ...
            for color_info in result_info:
                if color_info[0] == color:
                    index += 1
                    std_color_pic =[[[std_colors[color_info[0]][0]*255/100, 
                                      std_colors[color_info[0]][1]+128,
                                      std_colors[color_info[0]][2]+128]]]
                    RGB_pic = cv2.cvtColor(np.uint8(std_color_pic), cv2.COLOR_LAB2RGB)
