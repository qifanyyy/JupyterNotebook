...
lp3 = tk.Label(window, width=640, height=64)
lp3.place(x=160, y=368)

lcommon = tk.Label(window, text='共同色：', \
              font=('Arial', 12), width=16, height=2, justify=tk.LEFT)
lcommon.place(x=2+5, y=390)

ltable = tk.Label(window, textvariable=table, \
              font=('Arial', 12), width=75, height=4, justify=tk.LEFT)
ltable.place(x=48, y=480)
...
def main():
    ...
    # show image
    pic_array = cv2.cvtColor(common_uint8_lab.reshape(1, len(common_uint8_lab), 3),cv2.COLOR_LAB2RGB)
    pic_array = make_image(pic_array[0])
    pic = Image.fromarray(pic_array.astype('uint8')).convert('RGB')
    img = ImageTk.PhotoImage(pic)
    lp3.config(image=img)
    lp3.image = img
    window.update_idletasks()
    
    # show table
    ltable.config(height=len(common_color)+3)
    info = ' '*5 + 'LAB' + ' '*37 + 'A' + ' '*35 + 'B\n'
    for i in range(len(common_color)):
        info += str(common_color[i]) + ' '*25 + \
        str(round(100*common_area[i][0], 2)) + '%' + ' '*25 + \
