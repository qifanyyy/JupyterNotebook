...
dataset = panda.read_csv('bank-additional-full.csv', sep=';')

# Encode the classification labels to numbers
# Get classes and one hot encoded feature vectors
lebel_encoder = LabelEncoder()
lebel_encoder.fit(dataset['y'])
allClasses = lebel_encoder.transform(dataset['y'])
allFeatures = dataset.drop(['y'], axis=1)
...
toolbox.register("evaluate", getFitnessScore, X_train=X_train, X_test=X_test, y_train=y_train, y_test=y_test)
...
def getMetrics(hof):
    ...
    for individual in hof:
        ...
        validationAccuracy = getFitnessScore(individual, X_trainAndTest, X_validation, y_trainAndTest, y_validation)
...
if __name__ == '__main__':
    ...
    testAccuracy = getFitnessScore(individual, X_train, X_test, y_train, y_test)
    validationAccuracy = getFitnessScore(individual, X_trainAndTest, X_validation, y_trainAndTest, y_validation)
