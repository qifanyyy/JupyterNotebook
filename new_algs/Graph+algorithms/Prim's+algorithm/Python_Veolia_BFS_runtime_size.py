...
##print('len dict',len(list_keys))
##sys.exit()
NbNodes=len(list_keys)
i_cc = 0
graph_tempo = defaultdict(list)
for k,v in dict_Edges_Flags.items():
##        print('v',v,'v ended')
##        sys.exit()
        if i_cc<NbNodes:
                [graph_tempo.setdefault(k,[]).append(vi) for vi in v]  ##ok
                #graph_tempo.setdefault(k,[]).append(v[1])
                #graph_tempo.setdefault(k,[]).append(v[2])
                i_cc+=1
        else:
                break
graph_tempo = dict(graph_tempo)
print('graph_tempo',graph_tempo)
...
##NbNodes=10#5
i_counter=0
#for lk in list_keys[0:int(NbNodes/4)]:
for lk in list_keys[0:5]:  #ths aprt is linear
        i_counter +=1
        ##graph_new_Temp = dict_Edges_Flags
        ##data_set = dict_Edges_Flags.get(lk,[])
        Edges_to_close = bfs_paths_levels(graph_tempo,[lk,'0','open'],10)
        Edges_to_close = None
        graph_new_Temp = None
        print('lk',lk)
