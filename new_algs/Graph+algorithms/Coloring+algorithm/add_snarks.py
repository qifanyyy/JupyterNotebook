...
for n in gl:
    ...
    while stri != "":
        ...
        while True:
            ...
            engine.remove_specified_edge(edge)
            if not engine.edge_coloring():
                print ("edge: ", edge.get_endpoints(), " not critical")
                temp = graph_name + "_" + str(edge.get_endpoints())
                engine.layout()
                #engine.bicycle_layout()
                snark_graph_DB.add_graph(temp, g.to_json())
            engine.putback_for_specified_edge()
        stri = fin.readline()
