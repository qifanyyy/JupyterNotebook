...
def mutacion(sol, matrix):
    for i in range(len(sol)):
        adj_colors = []
        for j, other_node in enumerate(matrix[i]):
            if j != i and other_node == 1:
                if sol[j] not in adj_colors:
                    adj_colors.append(sol[j])

        if sol[i] in adj_colors:
            valid_colors = list(set(sol))            
            for adj_c in adj_colors:
                if adj_c in valid_colors:
                    valid_colors.remove(adj_c)
            
            if valid_colors:
                sol[i] = np.random.choice(valid_colors)
