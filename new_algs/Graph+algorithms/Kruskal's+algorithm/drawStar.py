...
# @author Kent Lee, star command by Katie Hummel
...
class StarCommand:
    def __init__(self, radius, width=1, penColor="black", fillColor="black"):
        self.radius = radius
        self.width = width
    ...
    def draw(self, turtle):
        ...
        turtle.begin_fill()
        for i in range(5):
            turtle.forward(100+self.radius)
            turtle.right(144)
        turtle.end_fill()

    def __str__(self):
        return '<Command radius="' + str(self.radius) + '" width="' + \
               str(self.width) + '" penColor="' + self.penColor + '" fillColor="' + self.fillColor +'">Star</Command>'
...
class DrawingApplication(tkinter.Frame):
    ...
    def buildWindow(self):
        ...
        def parse(filename):
            ...
            for commandElement in graphicsCommands:
                ...
                if command == "GoTo":
                    ...
                    fillColor = attr["fillColor"].value.strip()
                    cmd = StarCommand(radius, width, penColor, fillColor)
        ...
        def starHandler():
            cmd = StarCommand(float(radiusSize.get()), float(widthSize.get()), penColor.get(), fillColor.get())
            cmd.draw(theTurtle)
            self.graphicsCommands.append(cmd)
        ...
        starButton = tkinter.Button(sideBar, text="Draw Star", command=starHandler)
