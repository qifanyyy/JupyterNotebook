...
# Compute the Euclidean distance between each instance and cluster centers
def cal_eucldist(x, centroids):
    n = x.shape[0]  # the number of data points, or it represents the row of the data set
    eucldist = np.zeros((n, len(centroids))) #create zeros list for storing calculated the Euclidean distance
    for i in range(len(centroids)):
        eucldist[:, i] = np.linalg.norm(x - centroids[i], axis=1)
    return eucldist

# Assign each instances to the closest cluster center based on the minimum Euclidean distance
def assign_clusters(x, dist_table):
    n = x.shape[0]
    clusters = np.zeros(n) # create zeros list for storing the assignment
    clusters = np.argmin(dist_table, axis = 1)
    return clusters


# K-Means algorithm with Euclidean distance, x represents the data points, and k represents the number of clusters
def kmeans_eucldist(x, k):
    ...
    while error != 0:
        ...
        dist_table = cal_eucldist(x, centroids_new)
...
def generate_clusterlist (x, k):
    final_clusters = kmeans_eucldist(x, k)[0]
