...
def build_bayes_graph(im,labels,sigma=1e2,kappa=1):
	...
	for i in range(m*n):
		# add edge from source and to sink
...
def cut_graph(gr,imsize,algo):
	...
	# cut the graph #flows,cuts = maximum_flow(gr,source,sink)
	...
	cuts={}.fromkeys(gr.nodes(),1)
	
	in1=open("topy.txt",'r')
	lines=in1.readlines()
	in1.close()
	for it in lines:cuts[int(it)]=0
