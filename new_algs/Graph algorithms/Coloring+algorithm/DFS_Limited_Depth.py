...
class DFS_Limited_Depth:
    ...
    def solve(self, problem_instance, current_state, depth, max_depth):
        # print(len(self.ordered_state_array))
        # print(self.visited_states)
        # print("-----------------")
        if self.stop or depth >= max_depth:
            return
        current_state2 = None
            ...
            for child_state in tmp_childs:
                if not self.repetitive_state(child_state[0], problem_instance):
                    ...
                    if problem_instance.goal_state(child_state[0]):
                        ...
                        self.solve(problem_instance, child_state[0], depth + 1, max_depth)
                # else:
                #     print(child_state)
