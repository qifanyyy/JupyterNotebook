...
def find_all_shortest_paths(destination):
    ...
    pathsDict = {k:[] for k in unique_nodes}
    distDict = {l:[] for l in unique_nodes}
    #dest = destination
    ...
    for node in unique_nodes:
        ...
        for dest in destination:
            ...
            y = shortest_path(node,int(dest))[0]
            pack[dest] = y
            x=shortest_path(node,int(dest))[1]
            pack2[dest]= x
        pathsDict[node] = pack
        distDict[node] = pack2
...
desti =input('Enter your destinations: ')
desti =  list(desti.split(","))
...
#desti = [356, 924,194,491]
#print(desti, type(desti))
...
for i in ALLTHEDISTS.keys():
    ...
    if bestDest == 'None':
        ...
        bestPath = ALLTHEPATHS[i].get(bestDest)
        
    df.loc[len(df)]=[i,bestDest,min_dist[i],bestPath] 
                
df                

#nodes['GOAL_NODE'] = df.DESTINATION
#nodes['PATH_DISTANCE'] = df.DISTANCE
#nodes['DJK'] = df.PATH

#nodes.to_csv(nodesCSV+r'WithDJKandDistance.csv', index=False)""
#356, 66, 491, 924
