...
def write_stra_matC( myfile, coeff_idx, coeffs, idx, dim_name, dims, level ):
    ...
    add = 'std::array<T*,' + str(len(nonzero_coeffs)) + '> ' + mat_name + str(idx) + '_list = {'
    #add += ', '.join( [ str(c) for i, c in enumerate(coeffs) if is_nonzero(c) ] )
...
def write_straprim_caller(myfile, index, a_coeffs, b_coeffs, c_coeffs, dims, num_multiplies, level=1):
    ...
    write_stra_matAB( myfile, 0, a_coeffs, index, ['AC', 'AB'], dims, level )
    write_stra_matAB( myfile, 1, b_coeffs, index, ['AB', 'BC'], dims, level )
    write_stra_matC( myfile, 2, c_coeffs, index, ['AC', 'BC'], dims, level )
...
'''\
    ...
    //    Bv{0}.transpose();
    //    Cv{0}.transpose();
    //    straprim_ab<T,{1},{2},{3}>(comm, cfg, my_sub_len_AC, my_sub_len_BC,
    //             alpha,
    //             Bv{0},
    //             Av{0},
    //             beta,
